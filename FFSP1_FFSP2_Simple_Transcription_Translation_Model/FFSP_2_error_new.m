function [T,F,jump_times,E,Var,SD,Err_jump,E_tot,SD_tot,Err_tot] = FFSP_2_error_new(t_ob,Y,p0,C,Hidden_Species,delta,S,S_bis,n1,W_star,MAK,grad)
...
for i=1:length(t_ob)-1
    ...
    %[t_new,y]=ode1(@(t,y) Evolution_Probability_FFSP_2(t,y,C,Hidden_Species,Y(:,i),S_bis,mu,MAK,W_star,New,index_state,network), tspan, p0);
...
%                  rho_fsp=prop*y(q,kk)*(t_new(kk+1)-t_new(kk));
%                  int=int+rho_fsp;
%          
%          end
%      
...
%      end
    ...
    c_fsp_vector=zeros(size(t_new));
    c_fsp_vector(1)=1;

    for kk=1:length(t_new)-1
        int=0;
        %sum over all the state
        for q=1:Row_State
            % sum over all observed reactions
            prop=0;
            for rr=1:length(O)
                if MAK == 1 && network == 1
                    %[W] = propensity(X_new(a,:),Y(:,i),C,S_bis);
            ...
            int=int+prop*y(q,kk)*(t_new(kk+1)-t_new(kk));
         end
     
         c_fsp_vector(kk+1)=c_fsp_vector(kk)-int;
    end
    c_fsp=c_fsp_vector(end);
    
    % normalize the solution except the final one
    y(:,1:end-1)=y(:,1:end-1)./c_fsp_vector(1:end-1);
  ...
  for j=1:length(O_k)
    ...
    p_old=y(:,end);
    c_fsp_jump=sum(p_new)+grad*max(0,c_fsp-sum(p_old));
%    c_fsp-sum(p_old)
    p_new=p_new/c_fsp_jump;
    if sum(p_new) ~= 0
    y(:,end)=p_new;
    else 
        p_new=y(:,end);
